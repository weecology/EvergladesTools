...
@pytest.fixture()
def csv_data():
    df = nest_aggregate.load_classifications(CLASSIFICATIONS_CSV, min_version=min_version)
    return df
...
def test_run(download):
    nest_aggregate.run(CLASSIFICATIONS_CSV, min_version=min_version, download=download, generate=False,
