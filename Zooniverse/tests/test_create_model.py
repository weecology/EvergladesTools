...
PARSED_ANNOTATIONS_CSV = "data/parsed_annotations.csv"


# Setup method
@pytest.fixture()
def extract_images(tmpdir):
    aggregate.run(CLASSIFICATIONS_CSV, min_version=300, download=False, generate=False, savedir="data", debug=True)
    extract.run(image_data=IMAGE_DATA, classification_csv=PARSED_ANNOTATIONS_CSV, savedir="output")
...
def test_train_model(extract_images, annotations, tmpdir):
    ...
    create_model.train_model(train_path=train_path, test_path=test_path, save_dir=tmpdir, debug=True)
