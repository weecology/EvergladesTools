...
from Zooniverse import aggregate
from Zooniverse import extract
from .. import create_species_model
from .. import train as training_script
...
def extract_images(tmpdir):
    ...
    aggregate.run("{}/tests/data/everglades-watch-classifications.csv".format(root), min_version=300, download=False,
                  generate=False, savedir="{}/tests/output".format(root), debug=True)
    extract.run(image_data="{}/tests/data/everglades-watch-subjects.csv".format(root),
                classification_shp="{}/tests/output/everglades-watch-classifications.shp".format(root), savedir=tmpdir)
