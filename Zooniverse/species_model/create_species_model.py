...
def shapefile_to_annotations(shapefile, rgb_path, savedir="."):
    ...
    Returns:
        Dataframe: a csv file is written
    """
    # Read shapefile
    ...
    gdf["geometry"] = [box(int(left), int(bottom), int(right), int(top)) for left, bottom, right, top in
                       gdf.geometry.buffer(25).bounds.values]
...
def generate(shp_dir, empty_frames_path=None, save_dir="."):
    ...
    # Enforce rounding to pixels, pandas "Int64" dtype for nullable arrays
    ...
    # Save
    train_path = os.path.join(shp_dir, "train.csv")
    test_path = os.path.join(shp_dir, "test.csv")
    empty_test_path = os.path.join(shp_dir, "empty_test.csv")
...
if __name__ == "__main__":
    ...
    generate(shp_dir=shp_dir, empty_frames_path=empty_frames_path, save_dir=save_dir)
