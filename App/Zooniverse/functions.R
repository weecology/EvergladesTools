...
create_map <- function(colonies) {
  m <- leaflet(data = colonies) %>%
    addTiles() %>%
    addMarkers(popup = ~colony)
...
totals_plot <- function(selected_boxes) {
  ggplot(selected_boxes) +
    geom_bar(aes(x = species)) +
    coord_flip() +
    ggtitle("Project Total") +
    labs(x = "Label") +
...
site_totals <- function(selected_boxes) {
  ...
  to_plot <- selected_boxes %>%
    ...
    summarize(n = n()) %>%
    filter(site %in% selected_sites$site)
  ggplot(to_plot) +
    geom_col(aes(x = species, y = n, fill = site), position = position_dodge()) +
    coord_flip() +
    labs(x = "Label", y = "Count", fill = "Site") +
    theme(text = element_text(size = 20))
}

site_phenology <- function(selected_boxes) {
  to_plot <- selected_boxes %>%
    group_by(event, species, behavior) %>%
    summarize(n = n())
  ggplot(to_plot, aes(x = event, y = n, col = species, shape = behavior)) +
    geom_point(size = 5) +
    geom_line(size = 1) +
    labs(x = "Event", y = "Count", col = "label") +
    stat_smooth() +
    theme(text = element_text(size = 20))
}
...
plot_annotations <- function(selected_boxes, MAPBOX_ACCESS_TOKEN) {
  ...
  mapbox_url_template <- get_mapbox_url_template(mapbox_tileset, MAPBOX_ACCESS_TOKEN)

  m <- leaflet(data = selected_centroids) %>%
    addTiles(
      urlTemplate = mapbox_url_template,
      options = providerTileOptions(minZoom = 8, maxNativeZoom = 24, maxZoom = 24)
    ) %>%
...
behavior_heatmap <- function(selected_boxes) {
  class_totals <- selected_boxes %>%
    group_by(majority_class) %>%
    summarize(total = n())
  p <- selected_boxes %>%
    group_by(majority_class, behavior) %>%
    summarize(n = n()) %>%
    as.data.frame() %>%
    select(-geometry) %>%
    inner_join(class_totals) %>%
    mutate(prop = n / total * 100) %>%
    ggplot(.) +
    geom_tile(aes(x = majority_class, y = behavior, fill = n)) +
    scale_fill_continuous(low = "blue", high = "red") +
    labs(x = "Label", y = "Behavior", fill = "% of Label Total") +
...
time_predictions <- function(df) {
  ...
  site_names <- df %>%
    ...
    select(site, event) %>%
    group_by(site) %>%
    summarize(n = length(unique(event))) %>%
    filter(n > 1) %>%
    .$site
  df %>%
    group_by(site, event) %>%
    ...
    labs(y = "Predicted Birds", x = "Date") +
    theme(text = element_text(size = 20))
}

compare_counts <- function(df, selected_boxes) {
  automated_count <- data.frame(df) %>%
    select(site, event) %>%
    group_by(site, event) %>%
    summarize(predicted = n())
  zooniverse_count <- data.frame(selected_boxes) %>%
    select(user_name, site, event) %>%
    group_by(user_name, site, event) %>%
    summarize(Zooniverse = n())
  comparison_table <- automated_count %>%
    inner_join(zooniverse_count) %>%
    mutate(event = as.character(event)) %>%
    pivot_wider(names_from = user_name, values_from = Zooniverse)
...
get_mapbox_url_template <- function(mapbox_tileset, MAPBOX_ACCESS_TOKEN) {
  paste0(
...
plot_nests <- function(df, bird_df, MAPBOX_ACCESS_TOKEN) {
  ...
  m <- leaflet(data = df) %>%
    ...
    addCircles(
      data = bird_df, stroke = T, fillOpacity = 0, radius = 0.2, color = ~ species_colors(label),
...
zooniverse_complete <- function() {
  ...
  completed <- subject_data %>%
    group_by(Site) %>%
    mutate(annotated = subject_id %in% raw_annotations$subject_ids) %>%
    select(Site, subject_id, annotated) %>%
    group_by(Site, annotated) %>%
    summarize(n = n_distinct(subject_id)) %>%
    tidyr::spread(annotated, n, fill = 0) %>%
    mutate(Percent_Complete = `TRUE` / (`TRUE` + `FALSE`) * 100)
  p <- ggplot(completed, aes(x = Site, y = Percent_Complete)) +
    coord_flip() +
    geom_bar(stat = "identity") +
    labs(y = "Annotated (%)", x = "Subject Set")
  return(p)
}
